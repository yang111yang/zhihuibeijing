 ...
 * 网络缓存
 * 
 * @author 刘建阳 时间：2016-8-12下午2:45:37
 */
public class NetCacheUtils {
	...
	/**
	 * 三个泛型的意义： 第一个泛型：doInBackground的参数类型 第二个泛型：onProgressUpdate的参数类型
	...
	class BitmapTask extends AsyncTask<Object, Integer, Bitmap> {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			imageView.setTag(url);// 打标记，将当前的imageview和url绑定在了一起
		...
		protected void onPostExecute(Bitmap result) {
			...
			if (result != null) {
				// 给imageView设置图片
				// 由于listview的重用机制，导致imageview对象可能被多个item共用，从而可能将错误的图片设置给了imageview对象
